...
void ActionLoop::start(sf::RenderWindow& window, std::list<ActionSprite*>& actionSpritesPtrs, std::mutex& deletionMutex) {
	while (window.isOpen()) {
		checkEvents(window);
...
void ActionLoop::executeActions(std::list<ActionSprite*>& actionSpritesPtrs) {
	std::for_each(actionSpritesPtrs.begin(), actionSpritesPtrs.end(), [ &actionSpritesPtrs](ActionSprite* actionSpritePtr) {
		actionSpritePtr->action(actionSpritesPtrs);
		});
}

void ActionLoop::spriteCleanup(std::list<ActionSprite*>& actionSpritesPtrs, std::mutex& deletionMutex) {
